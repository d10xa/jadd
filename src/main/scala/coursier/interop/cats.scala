...
object cats extends LowPriorityCatsImplicits {
  ...
  implicit def coursierMonadFromCats[F[_]](implicit
    M: _root_.cats.Monad[F]
  ): Monad[F] =
    new Monad[F] {
      def point[A](a: A): F[A] = M.pure(a)
      def bind[A, B](elem: F[A])(f: A => F[B]): F[B] = M.flatMap(elem)(f)
    }
...
abstract class LowPriorityCatsImplicits extends PlatformCatsImplicits {
  ...
  implicit def coursierGatherFromCats[F[_], F0[_]](implicit
    ...
    new Gather[F] {
      def point[A](a: A): F[A] = N.pure(a)
      def bind[A, B](elem: F[A])(f: A => F[B]): F[B] = N.flatMap(elem)(f)
