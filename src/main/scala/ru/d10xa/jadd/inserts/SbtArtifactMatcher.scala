...
class SbtArtifactMatcher(val source: String) extends AnyVal {
  ...
  def find(artifact: Artifact): Seq[Match] = {
    ...
    def findBlockContents(s: String*): Seq[CodeBlock] =
      s.map(findBlockContent).reduce(_ ++ _)
    ...
    findInSequence(artifact).filter(inBlock) ++ findStandalone(artifact)
      .filterNot(inBlock)
  }
  ...
  def findStandalone(artifact: Artifact): Seq[Match] = {
    ...
    def regex0: Seq[Regex] =
      Seq(raw"""libraryDependencies\s\+=\s${r0(artifact)}""".r)

    def regex1: Seq[Regex] =
      Seq(raw"""libraryDependencies\s\+=\s${r1(artifact)}""".r)
    ...
    findMatches(regex0 ++ regex1 ++ regex2)
      .filterNot(isCommented)
      .map(Match.inSequence(_, b = false))
