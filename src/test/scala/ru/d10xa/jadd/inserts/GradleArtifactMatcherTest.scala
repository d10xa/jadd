...
class GradleArtifactMatcherTest extends FunSuiteLike with Matchers {
  ...
  test("basic dependency with compile configuration"){
    ...
    val match1: Seq[ArtifactView.Match] = matcher.find(Artifact("a:b"))
  ...
  test("basic dependency with testCompile configuration"){
    ...
    val match1: Seq[ArtifactView.Match] = matcher.find(Artifact("e:h"))
    match1.size shouldEqual 1
